...
@interface ViewController ()<UITableViewDelegate,UITableViewDataSource,DownLoadDelegate>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //02、检查缓存
        ...
        if (imagedata) {//内存里没有图片，再检查沙盒里有图片，就去读取沙盒里的图片
           ...
           SelfOperation *download=[self.opearationce objectForKey:item.icon];
            if (download) {
                NSLog(@"正在下载中");
            }else
            {
            SelfOperation *operation=[[SelfOperation alloc]init];
            operation.delegate=self;
            operation.indexpath=indexPath;
            operation.url=item.icon;
            [self.operationque addOperation:operation];
            [self.opearationce setObject:operation forKey:item.icon];
            }
        }
    }
    return cell;
...
#pragma mark - 下载完毕内存、沙盒缓存、刷新表格
- (void)downLoadOperation:(SelfOperation *)operation didFishedDownLoad:(NSData *)imageData{
    UIImage *image = [UIImage imageWithData:imageData];
    [self.imagecahe setObject:image forKey:operation.url];
     NSString *fullpath=[self getfullpathwithiconName:operation.url];
